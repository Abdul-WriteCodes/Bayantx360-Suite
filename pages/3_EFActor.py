"""
pages/3_EFActor.py — Factor Analysis Engine
─────────────────────────────────────────────────────────────────────────────
Wraps the original EFActor app.py into the Bayantx360 multipage structure.

Changes from standalone:
  • Auth/credit system replaced by utils.auth (shared module)
  • Session state keys use bx360_ prefix
  • deduct_credits() → spend_credits("efactor")
  • export_credit_cost() → credit_cost() from utils.auth
  • Sign-out calls utils.auth.logout()
  • All EFA/CFA analytical logic is unchanged
"""
...
from utils.auth import (
    ...
    spend_credits, refresh_credits, logout, credit_cost,
)
            ...
            if enter_btn:
                if not key_input.strip():
                    ...
                    with st.spinner("Validating key…"):
                        row = validate_key("efactor", key_input.strip())
                    if row is None:
...
# ── Refresh credits on load (paid users) ──────────────────────────────────────
if not st.session_state.get("bx360_is_free_trial", False):
    refresh_credits("efactor")
...
is_trial   = S.get("bx360_is_free_trial", False)
credits    = S.get("bx360_credits", 0)
key_owner  = S.get("bx360_key_owner", "Researcher")
access_key = S.get("bx360_access_key", "")
...
if S.efa_result is None:
    ...
    with col_e1:
        ...
        cost_clean = credit_cost(n_clean)
        ...
        if current_credits < cost_clean:
            ...
            if st.button(f"⬇ Download Cleaned Data ({cost_clean} cr)", use_container_width=True, key="dl_clean_btn"):
                new_bal = spend_credits("efactor", cost_clean)
        ...
        if syn_export is not None:
            ...
            if current_credits < cost_syn:
                ...
                if st.button(f"⬇ Download Synthetic Data ({cost_syn} cr)", use_container_width=True, key="dl_syn_btn"):
                    new_bal = spend_credits("efactor", cost_syn)
                    # credits synced by spend_credits()
        ...
        if has_cfa:
            ...
            if current_credits < REPORT_COST:
                ...
                if st.button(f"🔨 Build Word Report ({REPORT_COST} cr)", use_container_width=True, key="build_rpt"):
                    ...
                    new_bal = spend_credits("efactor", REPORT_COST)
                    # credits synced by spend_credits()
    ...
    zip_cost = credit_cost(zip_rows) + (REPORT_COST if S.report_docx else 0)
    ...
    if current_credits < zip_cost:
        ...
        if st.button(f"⬇ Build & Download ZIP ({zip_cost} cr)", use_container_width=True, key="dl_zip_btn"):
            ...
            zip_buffer.seek(0)
            new_bal = spend_credits("efactor", zip_cost)
            # credits synced by spend_credits()
